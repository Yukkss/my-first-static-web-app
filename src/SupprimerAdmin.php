...
<html>
	<head>
		<title>Titre : Suppression du compte admin</title>
		<meta charset="utf-8"/>
	</head>
	<body>
		...
		$local_idadmin= $_GET['P_refidadmin'] ; // création et affectation de la variable php avec l'info issue du formulaire de saisie
		echo $local_idadmin."<br />"; // affichage pour contrôle avec un retour à la ligne (balise <br />).

		$index=SupprimerAdmin($conn1,$local_idadmin); // appel de la fonction enregistreClient ; on passe en paramètre le nom du client (ici dans une variable)

		// fermeture de la connexion a la base de donnees.
		deconnexionBDD($conn1);
		header( "refresh:2;url=afficheAdmins.php" );
		?>
		Fin code de suppresion de la reservation <?php echo $local_idadmin ?>
		<br />
