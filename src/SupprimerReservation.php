...
<html>
	<head>
		<title>Titre : Suppression de la réservation</title>
		<meta charset="utf-8"/>
	</head>
	<body>
		...
		echo $local_idreservation."<br />"; // affichage pour contrôle avec un retour à la ligne (balise <br />).

		$index=SupprimerReservation($conn1,$local_idreservation); // appel de la fonction enregistreClient ; on passe en paramètre le nom du client (ici dans une variable)

		// fermeture de la connexion a la base de donnees.
		deconnexionBDD($conn1);
		header("refresh:2;url=afficheReservations.php");
		?>
		Fin code de suppresion de la reservation <?php echo $local_idreservation ?>
		<br />
